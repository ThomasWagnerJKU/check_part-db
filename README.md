...
Monitoring plugins for [Part-DB](https://github.com/Part-DB/Part-DB-server),
...
They query Part-DB's REST API (and, for the frontend check, its web UI), so
the monitoring host needs nothing but network access and an API token — no
database credentials and no agent on the Part-DB server.

| Plugin | Answers |
|---|---|
| `check_part-db-health` | Is the API up, is our token still accepted, does the database answer? |
| `check_part-db-web` | Does the web frontend serve its page, and is the TLS certificate still valid? |
| `check_part-db-stats` | How many parts, categories, footprints, suppliers … are in the database? |
| `check_part-db-stock` | Which parts have fallen below their configured minimum amount? |
...
Copy **all five files** into the plugin directory. `part_db.py` is the shared
library; the plugins import it from their own directory, so no `PYTHONPATH`
setup is needed, but they will not run without it.

```sh
install -m 0755 check_part-db-health check_part-db-web \
                check_part-db-stats check_part-db-stock \
                /usr/lib/nagios/plugins/
install -m 0644 part_db.py /usr/lib/nagios/plugins/
```
...
access is enough for every plugin.
...
The plugins look for the token in this order:
...
`check_part-db-web` needs no token at all — it checks the public frontend.

## Common options

All four plugins share the standard connection options:
...
| `-V`, `--version` | Show the plugin version and exit |
...
Every plugin has `--help` with its own options and worked examples.
...
- **UNKNOWN** — an operator or protocol problem: bad command line, no token
  configured, an unparseable response.

## The plugins

### check_part-db-health
...
reported Part-DB version, and TLS certificate expiry. `--cert-warning` and
`--cert-critical` are in days remaining (default 30 and 14);
`--no-cert-check` disables that part.
...
certificate cannot be read back, so reporting on it would be guesswork.
...
plenty, and keeps the load on Part-DB negligible.
...
which is why `check_part-db-stock` narrows the query with `minamount[gt]=0`
and compares locally instead of asking the API for "low" parts.

## Versioning

All plugins share one version number, reported by `-V`/`--version` and
defined in `part_db.py`. Releases are tagged `vMAJOR.MINOR`.
